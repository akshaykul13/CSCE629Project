...
public class MaxCapacityPathAlgo3 {
	...
	private static DirectedGraphAPI directedGraph;
...
//				System.out.println(edge);
	...
	private static void negateAllEdgeWeights(UndirectedGraphAPI graph, int type) {
		for(int i=0; i<graph.V(); i++){
			for(int j=0; j <graph.getAdj()[i].size(); j++){				
				if(type == -1){
					if(graph.getAdj()[i].get(j).getWeight() > 0){
						Edge newEdge = new Edge(graph.getAdj()[i].get(j).getV(), graph.getAdj()[i].get(j).getW(), graph.getAdj()[i].get(j).getWeight()*(-1));
						graph.getAdj()[i].set(j, newEdge);	
					}	
				}else{
					if(graph.getAdj()[i].get(j).getWeight() < 0){
						Edge newEdge = new Edge(graph.getAdj()[i].get(j).getV(), graph.getAdj()[i].get(j).getW(), graph.getAdj()[i].get(j).getWeight()*(-1));
	...
	public static void executeAlgorithm(DirectedGraphAPI graph) throws Exception{
		...
		long negateTime = System.nanoTime();
		System.out.println("First negate time = " + (negateTime-startTime));
		sortInAscendingOrder(directedGraph);
		long sortTime = System.nanoTime();
		System.out.println("Sorting time = " + (sortTime-negateTime));
		executeKruskalsAlgo(directedGraph);
		long kruskalTime = System.nanoTime();
		System.out.println("Kruskal time = " + (kruskalTime-sortTime));
		negateAllEdgeWeights(directedGraph, -1);	
	...
	private static void executeKruskalsAlgo(DirectedGraphAPI graph) throws Exception {
		dad = new int[graph.V()];
		for(int i=0; i<graph.V(); i++){
			dad[i] = i;
		}
...
//				System.out.println(edge);
	...
	private static void sortInAscendingOrder(DirectedGraphAPI graph) {		
		heap = new MinHeapAPI(graph.E());
		for(int i=0; i<graph.V(); i++){
			System.out.println("Sorting: Processing Adjacency List of vertex " + i);
			ArrayList<Edge> list = graph.getAdj()[i];
			for(Edge edge : list){
				if(!heap.exists(edge)){
					heap.insert(edge);
	...
	private static void negateAllEdgeWeights(DirectedGraphAPI graph, int type) {
		for(int i=0; i<graph.V(); i++){
			for(int j=0; j <graph.getAdj()[i].size(); j++){				
				if(type == -1){
					if(graph.getAdj()[i].get(j).getWeight() > 0){
